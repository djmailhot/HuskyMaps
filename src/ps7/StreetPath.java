...
public class StreetPath implements Path<StreetSegment, StreetPath> {
	...
	private StreetPath(StreetSegment node, StreetPath path, Set<StreetEntry> goals,
		...
		if(EnvironmentVariables.INTERSECTION_TRAVERSAL_ON  &&
				node.getStreetClass() != StreetClassification.PRIM_HWY &&
				node.getStreetClass() != StreetClassification.SEC_HWY &&
				...
				// node is considered an intersection on the graph and isn't a main highway.
		{
			// Having the intersection increase cost slows down pathfinding performance 
			length += (EnvironmentVariables.STREET_CLASSIFICATION_ON ? 
					parseStreetClass(node) : 2) * 
					EnvironmentVariables.INTERSECTION_TRAVERSAL_COST;
		}
	...
	private double calculateDistanceCostFactor(double distance){
    	double goalDistance = distance * (EnvironmentVariables.OPTIMIZE_FOR_PERFORMANCE_ON ? 2 : 1);
    	if(!EnvironmentVariables.INTERSECTION_TRAVERSAL_ON ){
    		return goalDistance;
    	}
    	...
    	return goalDistance * Math.max(distanceFactor, 
