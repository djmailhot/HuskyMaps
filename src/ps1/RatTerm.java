...
public final class RatTerm {
     ...
     * Addition operation.
     *
     * @requires (arg != null) && ((this.expt == arg.expt) || (this.isZero() ||
     *           arg.isZero() || this.isNaN() || arg.isNaN())).
    ...
    public RatTerm add(RatTerm arg) {
    	if(this.isNaN() || arg.isNaN()){
    		return NaN;
    	}else if(expt != arg.expt && !(this.isZero() || arg.isZero())){
    		throw new IllegalArgumentException();
    	}else{
    		return new RatTerm(coeff.add(arg.getCoeff()), this.isZero() ? arg.getExpt() : expt);
    	}
    }
    ...
    public RatTerm div(RatTerm arg) {
    	if(this.isNaN() || arg.isNaN() || arg == ZERO){
    		return NaN;
    	}else{
