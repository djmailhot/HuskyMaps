 ...
 * the other classes in package ps1. PolyGraph implements the graphing
 * component.
 *
...
public final class PolyGraph extends Canvas {
    ...
    CalculatorFrame calcFrame = null;
    ...
    /*
     * public void update() { if (img==null) { return; } // getChange marks the
     * new drawing spot. gi will only draw to here, // even if there is new
     * data! int scrollamt= gi.getChange(imgw, imgh-17); if (scrollamt< -imgw)
     * {return;} Graphics g= img.getGraphics(); if (scrollamt>-imgw) {
     * g.copyArea(0, 0, imgw, imgh-16, -scrollamt, 0); } if (scrollamt>0) {
     * g.setColor(Color.white); g.fillRect(imgw-scrollamt, 0, scrollamt,
     * imgh-16); g.setColor(zeroline); int y= gi.valueToY(0); if (y>0 && y<imgh-16) {
     * g.drawLine(imgw-scrollamt, y, scrollamt, y); } } else if (scrollamt<0) {
     * g.setColor(Color.white); g.fillRect(0, 0, -scrollamt, imgh-16);
     * g.setColor(zeroline); int y= gi.valueToY(0); if (y>0 && y<imgh-16) {
     * g.drawLine(0, y, -scrollamt-1, y); } } if (scrollamt>imgw) {
     * g.setColor(Color.lightGray); g.fillRect(0, imgh-16, imgw, 16); } //
     * System.out.println("Updating: scroll "+scrollamt); for (int i=0; i<gi.getNVars();
     * i++) { // getRecentData includes the last datapoint drawn // i.e. if
     * there is one new point, len=2 int len= gi.getRecentData(i); //
     * System.out.println(" "+gi.getVarName(i)+": "+len); // draw data
     * g.setColor(col[i]); g.drawPolyline(gi.getX(), gi.getY(), len);
     * g.drawString(gi.getVarName(i), i*60+1, imgh-3); } // draw info along
     * bottom g.setColor(Color.black); g.drawLine(0, imgh-16, imgw, imgh-16);
     * repaint(); }
     */
    ...
    public void paint(Graphics g) {
        ...
        if (img == null || w != imgw || h != imgh) {
            ...
            imgh = h;
            /*
             * if (gi.getNVars()!=0) { update(); return; }
             */
        }
